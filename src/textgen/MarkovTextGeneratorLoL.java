...
public class MarkovTextGeneratorLoL implements MarkovTextGenerator {
	...
	public void train(String sourceText)
	{
		...
		boolean lastNodeNeeded = true;
		for (int i = 1; i < list.length; i++) {
			boolean addNewNode = true;
			int index = -1;
			String currentWord = list[i];
			for (ListNode temp : wordList) {
				if ((temp.getWord()).equals(prevWord)) {
					addNewNode = false;
					index = wordList.indexOf(temp);
				}
				if (temp.getWord().equals(currentWord) && i == list.length - 1)
					lastNodeNeeded = false;
			}
			if (!addNewNode) {
				ListNode oldNode = wordList.get(index);
				oldNode.addNextWord(currentWord);
			}
			else {
		...
		ListNode linkBack = ((LinkedList<ListNode>) wordList).getLast();
		linkBack.addNextWord(starter);
	}
	...
	public String generateText(int numWords) {
		...
		while (i < numWords) {
			...
			ListNode correctNode = wordList.get(index);
			String randomWord = correctNode.getRandomNextWord(rnGenerator);
			output += randomWord + " ";
			currWord = randomWord;
			i++;
		}
